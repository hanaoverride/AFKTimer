...
#### Q: 메이플스토리를 플레이중인데도 불구하고 AFK 판정이 떠요!  

  게임 보안 프로그램이 마우스 입력과 출력을 가로채기 때문입니다.
  비슷한 메커니즘을 가진 다른 게임에서도 동일한 문제가 일어날 수 있습니다.
  이 문제를 해결하기 위해서는 약관 위반이 짙은 방법이 많아서, 해결까지 시간이 좀 걸릴 것 같습니다. 임시로 게임 도중에는 Pause를 눌러두는 방법을 채택하면 되겠습니다.

### 다운로드

레포지토리 우측의 'releases'를 확인해 다운로드 받아주세요. 

특별한 목적이 없는 한, 가장 최신 버전을 받으면 됩니다.
